...
def is_valid_process_title(title, sec_content):
    ...
    has_work_content = re.search(r'^\s*工作内容[:：]', sec_content, re.M | re.I)
    has_unit = re.search(r'^\s*计量单位[:：]', sec_content, re.M | re.I)
    has_table = re.search(r'<table>|J1-\d+', sec_content, re.I)
...
def query_quotas(db_path=None):
    ...
    try:
        ...
        print(f"{'序号':<4} {'定额编号':<10} {'所属工序':<25} {'计量维度':<20} {'计量值':<12} {'基价(元)':<12} {'人工费(元)':<12} {'材料费(元)':<12} {'机械费(元)':<12} {'合计工日':<10}")
        print("-" * 135)

        for idx, row in enumerate(rows, 1):
            process_id = row[0]
            process_name = row[1] if row[1] is not None else '未关联'
            quota_id = row[2]
            measurement_dimension = row[3] if row[3] is not None else '-'
            measurement_value = row[4] if row[4] is not None else '-'
            base_price = _to_number(row[5])
            labor_price = _to_number(row[6])
            material_price = _to_number(row[7])
            machine_price = _to_number(row[8])
            total_days = _to_number(row[9])
            ...
            if len(process_name) > 22:
                process_name = process_name[:22] + "..."

            print(
                f"{idx:<4} {quota_id:<10} {process_name:<25} {str(measurement_dimension):<20} {str(measurement_value):<12} "
                f"{display_base_price:<12} {display_labor_price:<12} {display_material_price:<12} {display_machine_price:<12} {display_total_days:<10}"
            )
