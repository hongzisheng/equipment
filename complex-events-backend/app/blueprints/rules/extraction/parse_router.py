...
def parse_single_file(file_name, file_bytes):
	...
	upload_resp = requests.put(file_urls[0], data=file_bytes, timeout=300)
...
def parse_document():
	...
	# 保存 markdown 到 assets/mdfile 目录
	base_name, _ = os.path.splitext(filename)
...
@parse_blueprint.route('/api/upload', methods=['POST', 'OPTIONS'])
def upload_file():
    if request.method == 'OPTIONS':
        return '', 204
...
@parse_blueprint.route('/api/files/<path:filename>', methods=['GET'])
