...
async function fetchWorkOrderTasks() {
  try {
    ...
    if (response.success && response.data) {
      ...
      console.log(`成功加载 ${response.data.length} 条工单任务数据`)
    } else {
      ElMessage.warning('未获取到工单任务数据')
    }
  } catch (error) {
...
async function fetchEquipmentInstances() {
  try {
    ...
    const response = await request({
      url: 'http://localhost:5000/api/equipment-instances',
      method: 'get'
    })
    // 重置树结构
    treeData.value[0].children = []
    const instances = response.data || response.equipment_instances || []
...
function deleteEquipment(row) {
  ElMessageBox.confirm(
    ...
    try {
      await request({
        url: `http://localhost:5000/api/equipment-instances/${row.id}`,
        method: 'delete'
      })
...
async function confirmImport(data) {
  try {
    ...
    // 发送POST请求到批量导入接口
    await request({
      url: 'http://localhost:5000/api/batch-import-equipment',
      method: 'post',
      data: requestData,
      // 监听请求进度
...
async function addEquipment() {
  ...
  try {
    const response = await request({
      url: 'http://localhost:5000/api/add-equipment',
      method: 'post',
      data: {
