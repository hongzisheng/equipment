    ...
    <el-card class="panel-card" shadow="hover">
      <div class="panel-header" @click="togglePanel('task')">
        <div class="panel-title">
          <img src="/src/assets/iconfont/任务管理.png" alt="工单管理" class="panel-icon mr6" />
          工单管理
        </div>
...
const fetchEquipmentTypes = async () => {
  try {
    const res = await request.get(`http://localhost:5000/api/equipment-types`)
...
const fetchEquipmentByType = async (typeId) => {
  try {
    const res = await request.get(`http://localhost:5000/api/equipment-instances/by-type/${encodeURIComponent(typeId)}`)
    console.log(typeId)
...
const fetchProcessTemplatesByType = async (typeId) => {
  try {
    const res = await request.get(`http://localhost:5000/api/process-templates/equipment-type/${encodeURIComponent(typeId)}`)
...
const submitNewWorkOrder = async () => {
  ...
  try {
    const res = await request.post('http://localhost:5000/api/manual-create-work-order', {
      equipment_id: newWorkOrderForm.equipmentId,
      process_template_ids: processIdsToSend  // 发送所有勾选的工序ID
    });
...
const refreshWorkOrders = async () => {
  ...
  try {
    ...
    if (response.data && Array.isArray(response.data)) {
      ...
      ElMessage.success(`成功加载 ${response.data.length} 条工单数据`)
    } else {
...
const refreshAllWorkOrderTasks = async () => {
  try {
    ...
    if (response.data && Array.isArray(response.data)) {
      allWorkOrderTasks.value = response.data
    } else {
