...
async function fetchTools() {
  try {
    ...
    const response = await request({
      url: 'http://localhost:5000/api/maintenance-tools',
      method: 'get'
    })
    
    if (response.success) {
      // 处理数据，将布尔值字段转换为是/否显示
      const processedTools = processToolData(response.maintenance_tools || [])
...
async function saveTool() {
  try {
    ...
    const response = await request({
      url: `http://localhost:5000/api/maintenance-tools/${currentTool.id}`,
      method: 'put',
      data: toolData
    })
    ...
    if (response.success) {
      ...
      ElMessage.error('更新机具信息失败: ' + (response.message || '未知错误'))
    }
  } catch (error) {
...
function deleteTool(row) {
  ElMessageBox.confirm(
    ...
    try {
      const response = await request({
        url: `http://localhost:5000/api/maintenance-tools/${row.id}`,
        method: 'delete'
      })
      ...
      if (response.success) {
        ...
        ElMessage.error('删除机具失败: ' + (response.message || '未知错误'))
      }
    } catch (error) {
...
async function confirmImport(data) {
  try {
    ...
    // 发送POST请求到批量导入接口
    const response = await request({
      url: 'http://localhost:5000/api/batch-import-maintenance-tools',
      method: 'post',
      data: requestData
    })
