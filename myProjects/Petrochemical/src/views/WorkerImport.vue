...
async function fetchWorkers() {
  try {
    ...
    // 处理数据，将is_certified字段值映射为是/否，并格式化compose
    const processedWorkers = processWorkerData(response.workers || [])
...
function deleteWorker(row) {
  ElMessageBox.confirm(
    ...
    try {
       await request({
        url: `http://localhost:5000/api/workers/${row.id}`,
        method: 'delete'
      })
...
async function confirmImport(data) {
  try {
    ...
    // 发送POST请求到批量导入接口
    await request({
      url: 'http://localhost:5000/api/batch-import-workers',
      method: 'post',
      data: requestData,
      // 监听请求进度
...
async function addWorker() {
  ...
  try {
    const response = await request({
      url: 'http://localhost:5000/api/add-worker',
      method: 'post',
      data: {
